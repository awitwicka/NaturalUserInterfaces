...
TexturedEffect::TexturedEffect(dx_ptr<ID3D11VertexShader>&& vs, dx_ptr<ID3D11PixelShader>&& ps,
	...
	: Effect(move(vs), move(ps)), m_cbProj(cbProj), m_cbView(cbView), m_cbModel(cbModel), m_cbMaterial(cbMaterial)
{ 
}
...
void TexturedEffect::Begin(const dx_ptr<ID3D11DeviceContext>& context)
{
	...
	ID3D11Buffer* vsb[3] = { 
		m_cbProj->getBufferObject().get(), m_cbView->getBufferObject().get(), m_cbModel->getBufferObject().get()
	};
